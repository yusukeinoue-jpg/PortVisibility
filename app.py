...
def assess_visibility_rank_v2(lat, lon):
    ...
    try:
        ...
        if not stations.empty:
            ...
            # 最寄りの駅名を特定
            # OSMのnameタグは "銀座駅" のように "駅" がつくことが多い
            # CSVデータは "銀座" なので、部分一致で検索する
            
            # データフレームに 'name' カラムがあるか確認
            nearest_station_name = "不明な駅"
            if 'name' in stations.columns:
                # 複数の駅がある場合、一番近いものを取得...したいが、
                # ここでは「最もランクが高い駅」を優先して採用するボーナスロジックにする
                        ...
                        for db_name in station_list:
                            if db_name in osm_name:
                                ...
                                if p_score > best_rank_score:
                                    ...
                                    best_rank = p_rank
                                    matched_name = db_name # CSV上の名前
                
                if best_rank:
                    score += best_rank_score
                    details.append(f"✅ 駅徒歩3分: **{matched_name}** ({best_rank}) (+{3.0+best_rank_score}点)")
                else:
                    # リストにない駅
                    if found_names:
                        details.append(f"✅ 駅徒歩3分: {found_names[0]} (ランク外) (+3.0点)")
                    else:
                        details.append(f"✅ 駅徒歩3分圏内 (+3.0点)")
            else:
                 details.append(f"✅ 駅徒歩3分圏内 (+3.0点)")
        else:
            details.append("・ 駅遠 (0)")
    except Exception as e:
        details.append(f"駅判定エラー: {e}")
    ...
    if st.button("判定開始", type="primary", key="single_btn"):
        if not user_input: st.error("入力してください")
            ...
            if coords:
                ...
                with col2: st.metric("スコア (Max 8.0)", f"{score}")
    ...
    **CSVファイルをアップロードしてください。** (推奨: URLまたは座標列)
    ...
    if uploaded_file:
        ...
        if st.button("一括判定を実行 (高速モード)", type="primary"):
            ...
            st.download_button("結果CSVをダウンロード", data=csv, file_name="scooter_ai_results_fast.csv", mime="text/csv")
